...
public unsafe class CoACD : MonoBehaviour
{
    ...
    public static List<Mesh> RunACD(Parameters parameters, Mesh mesh)
    {
        ...
        fixed (double* vptr = v) fixed (int* fptr = unityF)
        {
            ...
            using var res = Run(
                ref mi, parameters.threshold, parameters.maxConvexHull,
                (int)parameters.preprocessMode, parameters.preprocessResolution,
                parameters.sampleResolution, parameters.mctsNodes, parameters.mctsIteration, parameters.mctsMaxDepth, parameters.pca, parameters.merge, parameters.seed
            );
    ...
    public void GenerateCollisionMeshes(MeshFilter target)
    {
        ...
        var meshes = RunACD(parameters, baseMesh);
