...
namespace RFUniverse.Attributes.Digit
{
    ...
    public class DigitTargetObiRope : MonoBehaviour
    {
        ...
        {
            //初始化
            rod = GetComponent<ObiRod>();//从自身获取Obi组件
                                         //！！！！！！！！注册ObiSoftbody的碰撞委托事件
            rod.solver.OnCollision += ParticleCollision;

        }
        public List<int> currentParticle = new();
        ...
        void ParticleCollision(ObiSolver solver, ObiSolver.ObiCollisionEventArgs contacts)
        {
            ...
            foreach (var item in contacts.contacts)
            {
                ...
                {
                    currentParticle.Add(item.bodyA);
        ...
        //{
        //    if (rod)
        //        for (int i = 0; i < rod.particleCount; i++)
        //        {
        //            Gizmos.DrawSphere(rod.GetParticlePosition(i), 0.03f);
        //        }
        //}
        ...
        GameObject GetOrCreateRender(DigitAttr target)
        {
            ...
            copyRender.GetComponent<MeshFilter>().sharedMesh = rod.GetComponentInChildren<MeshFilter>().sharedMesh;
